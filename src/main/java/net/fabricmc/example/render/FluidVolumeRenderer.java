...
public class FluidVolumeRenderer {
    ...
    protected static RenderLayer getRenderLayer(FluidVariant fluid) {
        if (fluid == null) {
            ...
            return RenderLayers.getFluidLayer(fluid.getFluid().getDefaultState());
        }
    }
    ...
    protected static Sprite[] getSprites(FluidVariant fluid) {
        ...
        if (fluid != null) {
            ...
            if (handler != null) {
                Sprite[] sprites = handler.getFluidSprites(null, null, fluid.getFluid().getDefaultState());
            ...
            var textures = FluidVariantRendering.getSprites(fluid);

            // Seems odd that Sprite is AutoClosable... but there's nothing we can do about it
            //still = atlas.getSprite(textures[0]);
            //flowing = atlas.getSprite(textures[1]);
            still = textures[0];
            flowing = textures[1];
        }
    ...
    protected static void renderSimpleFluid(List<FluidRenderFace> faces, VertexConsumer vc, MatrixStack matrices,
                                            Sprite sprite, int colour) {
    ...
    protected static void vertex(VertexConsumer vc, MatrixStack matrices, double x, double y, double z, float u,
                                 float v, int r, int g, int b, FluidRenderFace f) {
     ...
     * @param f The source for the light and normal. */
    protected static void vertex(VertexConsumer vc, MatrixStack matrices, double x, double y, double z, float u,
                                 float v, int r, int g, int b, int a, FluidRenderFace f) {
     ...
     * @param nz Normal - Z */
    protected static void vertex(VertexConsumer vc, MatrixStack matrices, double x, double y, double z, float u,
                                 float v, int r, int g, int b, int light, float nx, float ny, float nz) {
     ...
     * @param nz Normal - Z */
    protected static void vertex(VertexConsumer vc, MatrixStack matrices, double x, double y, double z, float u,
                                 float v, int r, int g, int b, int a, int light, float nx, float ny, float nz) {
